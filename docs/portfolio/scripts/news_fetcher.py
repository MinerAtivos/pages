...
def get_ai_summary(ticker, context, is_priority=False):
    ...
    prompt = (
        f"Aja como um analista B3. Resuma de forma objetiva e em português as notícias de {ticker}. "
    ...
    try:
        ...
        if top_summaries:
            ...
            news_output["market_summary"] = g4f.ChatCompletion.create(
                ...
                messages=[{"role": "user", "content": f"Aja como um analista especialista em mercado de ações da B3. Resuma de forma objetiva e em português o clima do mercado B3 hoje:\n{combined}"}],
            )
    except: pass
